...
def build_feature_tables_for_date(client, report_date, phase, batter_raw_by_phase, pitcher_raw_by_phase, prior_offense, prior_offense_lhp, prior_offense_rhp, prior_defense, prior_pitchers, prior_batters):
    ...
    current_batters = m.aggregate_batter_quality(current_batter_raw)
    ...
    bullpen_profiles = bullpen.set_index('team').to_dict('index') if not bullpen.empty else {}
    return {
        'strengths': strengths,
        'current_batters': current_batters,
        'prior_batters': prior_batters,
...
def replay_clean_backtest(dates, output_path, summary_path, rebuild=False):
    ...
    prior_batters = m.previous_regular_season_batters(client, replay_year - 1)
