...
def previous_regular_season_team_offense(client, season_year):
    cache_path = season_cache_path('team_offense_prior', season_year)
    if os.path.exists(cache_path):
        try:
            return pd.read_csv(cache_path)
        except Exception:
            pass
    out = aggregate_team_offense(savant_statcast_csv(client, 'batter', season_year, dt.date(season_year, 3, 1), dt.date(season_year, 11, 30), 'R|'))
...
def aggregate_pitcher_quality(df):
    if df.empty:
        return pd.DataFrame(columns=['pitcher', 'pitcher_id', 'bf', 'xwoba_allowed', 'xslg_allowed', 'avg_ev_allowed', 'hardhit_allowed_rate', 'k_rate', 'bb_rate', 'pitcher_score'])
    work = df.copy()
...
def resolve_pitcher_projection(pitcher_name, pitcher_id, current_df, prior_df):
    ...
    if current is not None:
        ...
        if prior is not None:
            ...
            return {'score': (current_weight * current_score) + ((1.0 - current_weight) * finite(prior.get('pitcher_score'), 0.0)), 'source': 'current + prior blend', 'confidence': 0.55 + (0.45 * current_weight)}
        return {'score': current_score, 'source': 'current season', 'confidence': 0.60 if bf < STARTER_PRIOR_REGRESSION_BF else 0.95}
    if prior is not None:
        return {'score': finite(prior.get('pitcher_score'), 0.0), 'source': '2025 regular-season prior', 'confidence': 0.55}
    return {'score': 0.0, 'source': 'neutral prior', 'confidence': 0.25}
...
def predict_game(game, phase, weights, state, strengths, current_pitchers, prior_pitchers, bullpen_map, park_cache, client, venue_cache, margin_sigma):
    ...
    away_off, home_off = finite(away_row.get('offense_score_blended'), 0.0), finite(home_row.get('offense_score_blended'), 0.0)
    ...
    away_bullpen, home_bullpen = finite(bullpen_map.get(game['away_team']), 0.0), finite(bullpen_map.get(game['home_team']), 0.0)
    venue_meta = fetch_venue_metadata(client, game.get('venue_id'), game.get('venue_name'), venue_cache)
    park_history, park_static = park_history_adjustment(park_cache, game['scheduled_utc'].astimezone(LOCAL_TZ).date(), game.get('venue_id'), game.get('venue_name')), park_static_adjustment(venue_meta)
    weather = fetch_weather_context(client, game, venue_meta)
    weather_adj = weather_adjustments(weather)
    lineups = lineup_confirmation_counts(client, game['game_pk'])
    x_context = 0.04 * ((lineups.get('home', 0) - lineups.get('away', 0)) / 9.0)
    x_off, x_starter, x_bullpen = home_off - away_off, finite(home_starter['score'], 0.0) - finite(away_starter['score'], 0.0), home_bullpen - away_bullpen
    x_park, x_weather = finite(park_history['home_edge_adj'], 0.0), finite(weather_adj['home_edge_adj'], 0.0)
    ...
    away_runs_raw = base + (0.70 * away_off) - (0.50 * finite(home_starter['score'], 0.0)) - (0.22 * home_bullpen) + (0.50 * shared_total_adj) - (0.08 * x_context)
    home_runs_raw = base + (0.70 * home_off) - (0.50 * finite(away_starter['score'], 0.0)) - (0.22 * away_bullpen) + (0.50 * shared_total_adj) + (0.08 * x_context)
    away_runs_raw, home_runs_raw = float(np.clip(away_runs_raw, 2.0, 8.5)), float(np.clip(home_runs_raw, 2.0, 8.5))
    ...
    away_runs, home_runs = away_runs_raw * scale, home_runs_raw * scale
    ...
    return {
        ...
        'features': {'away_row': away_row, 'home_row': home_row, 'away_starter': away_starter, 'home_starter': home_starter, 'away_bullpen': away_bullpen, 'home_bullpen': home_bullpen, 'park_history': park_history, 'park_static': park_static, 'weather': weather, 'weather_adj': weather_adj, 'lineups': lineups, 'shared_env': shared_env, 'x_off': x_off, 'x_starter': x_starter, 'x_bullpen': x_bullpen, 'x_park': x_park, 'x_weather': x_weather, 'x_context': x_context, 'venue_meta': venue_meta},
    }
...
def main():
    ...
    backtest_log = load_backtest_log()
    ...
    strengths = blend_team_offense(
        season_offense,
        aggregate_team_offense(savant_statcast_csv(client, 'batter', report_date.year, report_date - dt.timedelta(days=30), pregame_end, game_types)),
        aggregate_team_offense(savant_statcast_csv(client, 'batter', report_date.year, report_date - dt.timedelta(days=14), pregame_end, game_types)),
        aggregate_team_offense(savant_statcast_csv(client, 'batter', report_date.year, report_date - dt.timedelta(days=7), pregame_end, game_types)),
