...
def previous_regular_season_batters(client, season_year):
    cache_path = season_cache_path('batter_priors', season_year)
    if os.path.exists(cache_path):
        try:
            return pd.read_csv(cache_path)
        except Exception:
            pass
    out = aggregate_batter_quality(savant_statcast_csv(client, 'batter', season_year, dt.date(season_year, 3, 1), dt.date(season_year, 11, 30), 'R|'))
    out.to_csv(cache_path, index=False)
    return out
...
def resolve_batter_projection(batter_name, batter_id, current_df, prior_df):
    ...
    current = current_by_id.get(int(batter_id)) if batter_id is not None and int(batter_id) in current_by_id else current_by_name.get(name_key)
    prior = prior_by_id.get(int(batter_id)) if batter_id is not None and int(batter_id) in prior_by_id else prior_by_name.get(name_key)
    if current is not None:
        ...
        if prior is not None:
            ...
            return {'score': (current_weight * current_score) + ((1.0 - current_weight) * finite(prior.get('batter_score'), 0.0)), 'source': 'current + prior blend', 'confidence': 0.45 + (0.55 * current_weight)}
        return {'score': current_score, 'source': 'current season', 'confidence': 0.50 if pa < BATTER_PRIOR_REGRESSION_PA else 0.92}
    if prior is not None:
        return {'score': finite(prior.get('batter_score'), 0.0), 'source': '2025 regular-season prior', 'confidence': 0.45}
...
def lineup_quality_context(game_pk, away_row, home_row, current_batters, prior_batters, client, lineup_counts_override=None):
    ...
    def side_context(team_row, players):
        if not players:
            return {'count': 0, 'avg_score': 0.0, 'adj': 0.0, 'avg_confidence': 0.0, 'prior_heavy': 0}
        projections, confidences, prior_heavy = [], [], 0
        ...
        adj = (mean_score - baseline) * confirmed_weight * (0.35 + (0.65 * avg_conf))
        return {
            'count': len(players),
            'avg_score': mean_score,
            'adj': float(np.clip(adj, -0.65, 0.65)),
...
def predict_game(game, phase, weights, state, strengths, current_pitchers, prior_pitchers, current_batters, prior_batters, bullpen_map, park_cache, client, venue_cache, margin_sigma, lineup_counts_override=None):
    ...
    lineup_quality_component = 0.06 * finite(lineup_ctx.get('delta'), 0.0)
    x_lineup = lineup_count_component + lineup_quality_component
    lineup_multiplier = lineup_multiplier_from_state(state, phase)
    away_lineup_adj = finite(((lineup_ctx.get('away') or {}).get('adj')), 0.0)
    home_lineup_adj = finite(((lineup_ctx.get('home') or {}).get('adj')), 0.0)
    ...
    x_context = x_context_core + (lineup_multiplier * x_lineup)
    ...
    logit_no_lineup = (weights['w_off'] * x_off) + (weights['w_starter'] * x_starter) + (weights['w_bullpen'] * x_bullpen) + (weights['w_park'] * x_park) + (weights['w_weather'] * x_weather) + (weights['w_context'] * x_context_core) + weights['home_field']
    logit_full_lineup = (weights['w_off'] * x_off) + (weights['w_starter'] * x_starter) + (weights['w_bullpen'] * x_bullpen) + (weights['w_park'] * x_park) + (weights['w_weather'] * x_weather) + (weights['w_context'] * (x_context_core + x_lineup)) + weights['home_field']
    ...
    away_runs_raw = base + (0.70 * away_off) - (0.50 * finite(home_starter['score'], 0.0)) - (0.22 * home_bullpen) + (0.50 * shared_total_adj) + (0.06 * context_feature_mult * finite(away_schedule.get('score'), 0.0)) - (0.10 * context_feature_mult * home_def) + (0.12 * lineup_multiplier * away_lineup_adj) - (0.08 * lineup_multiplier * lineup_count_component)
    home_runs_raw = base + (0.70 * home_off) - (0.50 * finite(away_starter['score'], 0.0)) - (0.22 * away_bullpen) + (0.50 * shared_total_adj) + (0.06 * context_feature_mult * finite(home_schedule.get('score'), 0.0)) - (0.10 * context_feature_mult * away_def) + (0.12 * lineup_multiplier * home_lineup_adj) + (0.08 * lineup_multiplier * lineup_count_component)
    ...
    return {
        ...
        'features': {
            ...
            'weather': weather, 'weather_adj': weather_adj, 'lineups': lineups, 'lineup_ctx': lineup_ctx, 'shared_env': shared_env, 'lineup_multiplier': lineup_multiplier, 'probability_shrink_alpha': probability_shrink_alpha, 'p_home_model': p_home_model, 'p_home_simple': p_home_simple, 'total_model': total_model, 'total_simple': total_simple, 'total_shrink_alpha': total_shrink_alpha, 'margin_model': margin_model, 'margin_simple': margin_simple, 'margin_shrink_alpha': margin_shrink_alpha,
            ...
            'context_components': {'lineup_count': lineup_count_component, 'lineup_quality': lineup_quality_component, 'schedule': schedule_component, 'defense': defense_component}, 'x_lineup': x_lineup, 'x_context_core': x_context_core,
